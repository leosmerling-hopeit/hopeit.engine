...
class Message:
    ...
    def payload(self) -> bytes:
        return b64decode(self.b64value.encode())
...
class Queue:
    def __init__(self, maxlen: int):
        ...
        self.data = deque(iterable=[None] * self.maxlen, maxlen=self.maxlen if self.maxlen else None)
        self.producer_ids: Dict[str, int] = defaultdict(int)
        self.consumer_offsets: Dict[str, int] = defaultdict(int)
        self.consumer_ids: Dict[str, Set[str]] = defaultdict(partial(defaultdict, int))
        self.offset0 = -1
...
async def consume_in_process(stream_name: str, consumer_group: str, consumer_id: str,
    ...
    for _ in range(2):
        items, missed = await q.consume(consumer_group, consumer_id, int(batch_size))
        if len(items) > 0:
            break
        await asyncio.sleep(int(timeout_ms) / 1000.0)
